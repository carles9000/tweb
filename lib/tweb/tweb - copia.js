...
function TWebBrowse( cId, aData, lInit ) {
	...
	this.SetEdit = function( lOnOff ) {
		...
		var lOldValue = this.lEdit
		
		if ( typeof lOnOff != 'boolean' )
			return lOldValue
		
		console.log( 'Old', lOldValue )
		console.log( 'New', lOnOff )
	...
	this.UpdateChanges = function( cIdRow, cAction, row  ) {	
		...
		for (i = 0; i < TWebBrowse.aChanges.length; i++ ) {
		
			if ( TWebBrowse.aChanges[i][ 'id' ] == cIdRow ) {
		...
		if ( nPos >= 0 ) {
			TWebBrowse.aChanges[nPos][ 'action' ] = cAction
			TWebBrowse.aChanges[nPos][ 'row' ] = row
		} else {
			TWebBrowse.aChanges.push( { 'action' : cAction, 'id' : cIdRow, 'row' : row } )
		}
			
		console.log( 'aChanges', TWebBrowse.aChanges )	
	...
	/*
		https://examples.bootstrap-table.com/#methods/update-row.html#view-source	
      ...
      $table.bootstrapTable('updateRow', {
		...
		https://examples.bootstrap-table.com/#methods/update-by-unique-id.html#view-source	
      ...
      $table.bootstrapTable('updateByUniqueId', {
		...
		https://examples.bootstrap-table.com/#methods/update-cell.html#view-source
	...
	*/
	...
	/* Exemple  de extreure index a partir de 1 checkbox
		$('#show').click(function () {
		  var index = []
...
*/	
	...
	this.DeleteRow = function( cField, acValues ) {		
	
	//data-unique-id="id"
	//$table.bootstrapTable('getRowByUniqueId', 1).
	
		if ( typeof acValues != 'array' )
			acValues = [acValues]
		
		var aDat = { 'field' : cField, 'values' : acValues }		
		...
		for (i = 0; i < acValues.length; i++) {
			...
			this.UpdateChanges( cIdRow, 'D', aData[ acValues[i] ]  )		
		}
		
		oBrowse.bootstrapTable('remove', aDat )					
	}
	
	
	this.GetRow = function( index ) {
		return oBrowse.bootstrapTable('getData')[index]
	}
	...
	this.PostEdit = function( row ) {	
	
		console.log( 'PostEdit', row )
	}
	
	this.Edit = function( e, row, element, cField ) {	
	
		var index 		= element.index()
		var aCfgCols 	= TWebBrowse.aCfgCols[ cId ];

			
		//var fn =  window[ oSelf.bPostEdit ]
		
		this.nLastIndex = index; 						
		console.log( 'TITLE', oSelf.cTitle  )
		var o = new TWebForm( TWebBrowsePostEdit , index, oSelf, aCfgCols, oSelf.cTitle  );
	...
	this.Edit2 = function( e, row, element, cField ) {	
		...
		var o = new TWebForm( TWebBrowsePostEdit , index, oSelf, aCfgCols );
	...
	this.Init = function( aData ) {	    	    		
		...
		console.log( 'Init TWebBrowse ' + this.cId, this )		
		console.log( 'Init TWebBrowse2 ', this )		

			
	    oBrowse = $('#' + this.cId )
		
		TWebBrowse.aChanges[ this.cId ] = [];
	    
	    if ( typeof this.bClick == 'function' ){
	        oBrowse.off('click-row.bs.table', this.bClick )
	        oBrowse.on('click-row.bs.table', this.bClick )
	    }
		
        oBrowse.off('dbl-click-row.bs.table', this.bDblClick )
		
		if ( this.lEdit ) {

			if ( this.bEditor != '' ) {
			
				var fn =  window[ this.bEditor ]
					
				if (typeof fn === "function") 
					oBrowse.on('dbl-click-row.bs.table', fn )
				else
					oBrowse.on('dbl-click-row.bs.table', this.Edit )				
	    ...
	    } else if ( typeof this.bDblClick == 'function' ){
